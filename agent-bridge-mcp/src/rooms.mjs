...
/** Next founder op sequence (decimal string), persisted; monotonic. */
export function nextFounderSeq(room_id) {
  const store = loadRooms();
  const room = store.rooms[room_id];
  if (!room) throw new Error(`unknown room ${room_id}`);
  const cur = Number(room.founder_seq_next ?? "0");
  room.founder_seq_next = String(cur + 1);
  saveRooms(store);
  return String(cur);
}
