...
test("buildRoomMsgBody stamps identical cross-check fields for the whole fan-out", () => {
  ...
  const body = buildRoomMsgBody({ room_id: "r1", members,
    roster_digest: "deadbeef", sender_seq: 7, mentions: ["AIR-CODX"], text: "hello team" }); // NUMBER seq
  ...
  assert.equal(body.sender_seq, "7"); // String() coercion of the number 7
