...
**Carried into later milestones:** encrypted sidecar (post-v1 startup opt, gated on the Task-9 budget) · graph-proximity boost in recall (M3) · cross-encoder `bge-reranker` impl behind the trait (post-v1) · `EmbeddingGemma` upgrade + `sqlite-vec` opt-in (deferred) · per-platform ort offline-bundling CI proof before bge-small can be the shipped default · `schema_version` format-gating logic (field reserved now) · desktop surfacing of the §15 model-switch integrity signal (M7) · **incremental `index_event` affordance** (add one derived vector to the live index + `keyword_add`, so appends don't need a full `rebuild_indexes` — M7; whole-impl review) · **FastEmbed (bge-small) recall@k comparison run** (the R1 embedder tiebreak is currently one-sided: model2vec measured at recall@3=1.0, bge-small deferred — needs the ONNX run) · **`cargo-deny` CI guard** (deny.toml banning `onig`/`onig-sys`/`ort`/`ort-sys`) to lock the pure-Rust-default R1 guarantee across `cargo update`s (spec §8.6/§11; the offline story is one transitive feature-unification from silent violation).
