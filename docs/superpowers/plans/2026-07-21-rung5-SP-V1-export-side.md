...
> 1. **`class↔field-set` invariant — EMPIRICALLY PROVEN LOAD-BEARING (Task-5 review, not optional hardening).** The flat `AirmemItem` type permits illegal combinations. Verify MUST reject: `Stamped` ⟹ `event_bytes`+`signature` PRESENT and `content`/`display` ABSENT; `SealVouched` ⟹ the inverse (`content` present, `event_bytes`/`signature`/**`carried_origin`** absent). Add a `Malformed{detail}` case + a tamper-matrix row for each violation. Also enforce kind↔class: `note`⟹Stamped, `session`/`ingest`/`dossier`⟹SealVouched.
>    **The proof (Task-5 reviewer, mutation-run against a built bundle):** injecting `carried_origin: Some("external")` into a **SealVouched** item leaves BOTH `seal_ok == true` AND `root_ok == true`. `carried_origin` is Merkle-excluded by design, and a SealVouched item has no `event_bytes`, so the A5 origin cross-check has no substrate to compare against. **The `carried_origin`-absent-on-SealVouched clause is the SOLE enforcement point** — without it an attacker can paint a weak, export-time-vouched item (a session/ingest/dossier) with the strong "external" write-time attestation token, invisibly to every cryptographic commitment in the format. Same review confirmed `items[].leaf` is likewise uncommitted (freely rewritable with seal+root still green) ⇒ verify MUST recompute every leaf and compare, never trust the stored hex (`ItemHashMismatch(i)`).
...
> 4. **REJECT `items: []` (and `item_count == 0`) as `Malformed` BEFORE any Merkle call** (Task-3 review). `merkle::root()` opens with `assert!(!leaves.is_empty())`, and `assert!` is NOT compiled out in release — once verify calls it, a malicious `.airmem` with an empty items array becomes an unconditional panic (a trap/abort in the WASM verifier instead of a clean ❌). Also cross-check `manifest.item_count == items.len()` before hashing. NOTE: `merkle.rs:21`'s doc cites `EmptySelection` as the upstream guard — that variant does not exist yet; the guard is currently a promise, not code.
> 5. **`display`-key charset constraint** (Task-3 review, proven): `serde_jcs 0.1.0` sorts object keys by UTF-8 bytes, but RFC-8785 §3.2.3 mandates UTF-16 code-unit order — they diverge for any non-BMP key, and **every emoji is non-BMP**. The only free-form object in the format is `AirmemItem.display`. Self-consistent for us (build + native verify + the WASM verifier are all the same Rust crate), so this is NOT a security hole — it is a FOREIGN-verifier conformance landmine. Enforce ASCII-only `display` keys at build (Task 5/7 — we author those keys: `when`, `source`, …) and reject non-ASCII `display` keys in verify. Record the constraint in the format doc so Task 11's vectors can never encode a divergent bundle.
> 6. **Any future inclusion-proof API must bind `(item_count, index)`** and derive the expected path shape from `n` — under odd-node promotion, node positions differ per `n`, so a "fold the path upward" verifier that trusts caller-supplied direction bits is shape-agnostic. (Well-formedness, not a forgery hole: domain separation already prevents forging membership of a new item.) Relevant when the deferred sub-share UX lands.
> 7. **[STATUS 2026-07-29: DEFERRED TO SP-V2 — an explicit prerequisite of the HTTPS resolver, not an oversight.]** Task 6 implemented the mandate half (never present an L1 pass as identity evidence; `IdentityLevel::UnverifiedOffline`) and deferred `verify_strict` itself with written rationale at `verify.rs:189`. Reasons accepted by the controller: (a) it needs `bossclaw-canon` to expose its currently-private `Signature` type — a cross-crate change; (b) the weak-key branch is only reachable via a small-order-point signature forgery that could not be constructed cheaply, so implementing it in SP-V1 would have shipped UNTESTED security code; (c) **L2 is inert in SP-V1** — `OfflineResolver` is the only resolver that ships, so no production path reaches that arm. **SP-V2 MUST take it up with the HTTPS resolver**, in the same change that first makes L2 live, and must land it with a real weak-key test vector. Original note follows.
>    **Consider `verify_strict` at L2, AFTER the registry raw-byte comparison** (Task-4 review, empirically confirmed): ed25519-dalek 2.2.0's `VerifyingKey::from_bytes` returns `Ok` for all four small-order points (`is_weak() == true`) and even for non-canonical `[0xff; 32]`, and `verify_bytes` uses the non-strict `Verifier::verify`. **Harmless at L1 by design** — the embedded key is attacker-chosen and L1 carries ZERO identity assurance, so signing with a weak key you already control gains nothing. It matters only if code ever treats "L1 passed" as evidence. At L2, once the registry lookup has pinned the key by raw bytes, strict verification costs nothing and closes the class. Also: **never let any surface present an L1 pass as identity evidence** (already §2.5 copy, restated here as an implementation invariant).
...
> **CHECKLIST ADDITIONS from the Task-5 review (2026-07-29) — do not lose these between tasks:**
> - **ASCII-only `display` keys are enforced HERE** (Task-6 carry-forward #5 names "Task 5/7"; Task 5 documents the invariant on `ItemInput::SealVouched`, it does not enforce it). Reviewer verified this task's sketched call-site uses ASCII literal keys (`title`/`project`/`tool`) so it is satisfied by construction today — make it an explicit assertion or a debug-checked helper so a future added key cannot silently break foreign-verifier conformance. Keys only; values are free-form.
> - **`build_bundle` PANICS on an empty `items` vec** (via `merkle::root`'s assert; confirmed by `catch_unwind`). This task is the caller — it MUST reject the empty selection (`EmptySelection`) BEFORE calling build. Owner-side data, so this is a precondition to honor, not a DoS (the verify-side twin of the same assert IS a DoS — carry-forward #4).
