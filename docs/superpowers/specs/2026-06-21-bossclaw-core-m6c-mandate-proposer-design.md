...
1. **Self-write → watcher feedback loop** 🌀 (HIGH; A6/§8). `execute_write` mutates the target but does **not** re-ingest; a live watcher could see the engine's own write as a change and re-fire forever. **Three guards, layered:**
   ...
   - *Structural (now MANDATORY, Rev 2 / finding A):* the target **must live outside every active read-grant root**, *enforced at `add_mandate`* (§4.2) — not a "should." Then the engine's own confirmed write can never fire a source event nor be re-ingested as a source, so the recipe can never fold its output into its input. This makes convergence hold *unconditionally* and demotes the Belt guard below to true belt-and-suspenders.
