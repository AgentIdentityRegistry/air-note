...
- **`default-features = false` made durable** — a `bossclaw-core`-scoped CI guard (`cargo tree` on the default-feature graph) fails the build if `hf-hub`/`ureq`/`reqwest` ever enter it, so the "embedder is network-free → needs no sandbox" property cannot silently regress (security finding #1). *(A workspace-wide cargo-deny ban was rejected during planning: the desktop crate legitimately depends on `reqwest`.)*
...
- The network-free guard below is a `cargo tree` check in CI (no new tooling).
...
- **In-process embedder, no sandbox needed — and the property is locked.** `Model2Vec` is pure-Rust static embeddings: safetensors load (no pickle/`torch.load` gadget), no subprocess, no network. Confirmed at the dependency boundary — `bossclaw-core` declares `model2vec-rs` with `default-features = false`, so its `hf-hub`/`ureq`/`reqwest` fetch path is **not compiled** (`cargo tree -i hf-hub` ⇒ absent from the engine graph). To keep this from silently regressing (a future edit re-enabling defaults would compile a network fetcher into the engine and invalidate the no-jail justification), SP2 adds a **`bossclaw-core`-scoped CI guard** — `cargo tree -p bossclaw-core` on the default-feature graph fails the build if `hf-hub`/`ureq`/`reqwest` appear. (Scoped to the engine, not the workspace: the desktop legitimately depends on `reqwest`.) This is *why* native+model2vec needs no jail (unlike markitdown/M5b's Python venv).
...
- New CI step (`.github/workflows/build.yml`, `bossclaw-core` job): a `cargo tree`-based "engine network-free" guard.
