...
- **Dirty set:** an entity is a (re)summary candidate this tick iff it was minted, or is an endpoint of a `link`/`invalidate` emitted this tick. Tracked in `evolve_once` (the loop already resolves every mention to an `entity:<ulid>` and knows which edges it emitted). Bounded to `SUMMARY_BATCH`.
...
- **Append error** (signing/chain/empty-source rejection) on the `supersede` or `page` propagates; a partial (supersede emitted, page failed) is avoided by emitting page first into a staged value and only superseding once the page append is known-good — **order: build+validate page → append `supersede` → append `page`**, and if the `page` append fails after the `supersede`, that is surfaced (a topic left with no current page is recoverable next tick by re-summarizing). *(Open item flagged for the second-opinion review: confirm this ordering vs. an atomic two-append helper.)*
...
| `SUMMARY_BATCH` | `8` | Max topics (re)summarized per tick (bounds tick latency; ≤ the entities a `EVOLVE_BATCH`-memory tick can touch). |
| `MAX_CLAIMS_PER_PAGE` | `32` | Resource cap on claims accepted from one draft (mirrors `MAX_ENTITIES_PER_MEMORY`). |
| `SUMMARY_REFLECT` | `2` | Compose + one critique pass (bounds model calls per page; mirrors `MAX_REFLECT`). |
| `PAGE_RECALL_WEIGHT` | `1.0` | Multiplicative tilt on a page hit's fused score; `1.0` = neutral. Tunable (§7). |
