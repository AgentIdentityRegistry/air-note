...
- **Trust boundary — same-uid, all ops (M1a).** The boundary is the user account, enforced twice: the `0600` socket excludes other uids at the FS layer, and the daemon's accept loop additionally verifies the peer's credentials (`SO_PEERCRED` on Linux / `LOCAL_PEERCRED` on macOS) and rejects any connection whose uid differs from the daemon's own. Within that boundary the surface is deliberately flat: **any same-uid process can invoke all wire ops — including the `EnableCloudReasoner` egress probe and `Teardown`** (the same power a same-uid process already holds over the app's data dir and keychain items today). Per-op authorization/entitlements are deferred to M1b, when the first non-app clients (Claude Code) arrive.
