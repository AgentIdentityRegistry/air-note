// Copied from apps/desktop/src-tauri/src/engine/ollama_probe.rs (M1a Task 4); the in-app original is removed in Task 6.

//! Ollama detection. A plain `GET` of the LOCAL Ollama's `/api/tags` over `reqwest`.
//!
...
/// Short connect/read budget: a missing Ollama should fail fast, not hang the scheduler.
...
/// `{ reachable: false, model_present: false }` (fail-closed).
